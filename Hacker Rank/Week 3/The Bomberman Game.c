...
int r = grid_count;
    ...
    *result_count = r;
    ...
    char** firstDetonation = malloc(r * sizeof(char*));
    char** secondDetonation = malloc(r * sizeof(char*));

    for (int i = 0; i < r; i++) {
        fullGrid[i] = malloc((c + 1) * sizeof(char));
        firstDetonation[i] = malloc((c + 1) * sizeof(char));
        secondDetonation[i] = malloc((c + 1) * sizeof(char));
    }
    for (int i = 0; i < r; i++) {
        for (int j = 0; j < c; j++) {
            fullGrid[i][j] = 'O';
    ...
    for (int i = 0; i < r; i++) {
        for (int j = 0; j < c; j++) {
            firstDetonation[i][j] = 'O';
        }
        firstDetonation[i][c] = '\0';
    }
    ...
    for (int i = 0; i < r; i++) {
        for (int j = 0; j < c; j++) {
            if (grid[i][j] == 'O') {
                firstDetonation[i][j] = '.';
                if (i > 0) firstDetonation[i - 1][j] = '.';
                if (i < r - 1) firstDetonation[i + 1][j] = '.';
                if (j > 0) firstDetonation[i][j - 1] = '.';
                if (j < c - 1) firstDetonation[i][j + 1] = '.';
            }
        }
    }
    for (int i = 0; i < r; i++) {
        for (int j = 0; j < c; j++) {
            secondDetonation[i][j] = 'O';
        }
        secondDetonation[i][c] = '\0';
    }

    for (int i = 0; i < r; i++) {
        for (int j = 0; j < c; j++) {
            if (firstDetonation[i][j] == 'O') {
                secondDetonation[i][j] = '.';
                if (i > 0) secondDetonation[i - 1][j] = '.';
                if (i < r - 1) secondDetonation[i + 1][j] = '.';
                if (j > 0) secondDetonation[i][j - 1] = '.';
                if (j < c - 1) secondDetonation[i][j + 1] = '.';
            }
        }
    }
    if (n == 1) {
